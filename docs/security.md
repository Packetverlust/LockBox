...
LockBox uses multiple layers of protection. Your master password never touches disk — ever.
...
Most tools use PBKDF2 for key derivation. LockBox uses Argon2id — the winner of the Password Hashing Competition and the current industry standard.
...
Parameters used:
- Time cost: 3 passes
- Memory cost: 64MB per guess
- Parallelism: 4 threads
- Output: 32 bytes → used as AES key

## What an attacker needs

To crack your vault they need **both files**:
- `~/.lockbox/vault.enc`
- `~/.lockbox/salt.bin`
