...
def printHelp() -> None:
    ...
    cmds.add_row("update-lockbox", "Check for updates and install the latest version.")
...
@app.command(name="update-lockbox")
def updateLockbox(
    check_only: bool = typer.Option(False, "--check-only", help="Only check for updates, don't install."),
) -> None:
    """Check for a newer LockBox release on GitHub and install it."""
    import urllib.request
    import json

    header("update-lockbox")

    try:
        req = urllib.request.Request(
            "https://api.github.com/repos/packetverlust/lockbox/releases/latest",
            headers={"User-Agent": "lockbox-updater"},
        )
        with urllib.request.urlopen(req, timeout=10) as resp:
            release = json.loads(resp.read())
    except Exception as e:
        console.print(Panel(f"[red]Could not reach GitHub: {e}[/red]", border_style="red", box=box.ROUNDED))
        raise typer.Exit(1)

    latestTag = release.get("tag_name", "")
    latest = latestTag.lstrip("v")
    current = __version__
    ...
    if latest == current:
        console.print(Panel("[green]You are already on the latest version.[/green]", border_style="green", box=box.ROUNDED))
        return

    if check_only:
        console.print(Panel(
            f"[yellow]Update available: {latestTag}[/yellow]\n\nRun [bold cyan]lockbox update-lockbox[/bold cyan] to install.",
            border_style="yellow", box=box.ROUNDED,
        ))
        return
    ...
    if isWindows:
        asset = next((a for a in assets if "windows" in a["name"].lower() or a["name"].endswith(".exe")), None)
    else:
    ...
    if asset:
        ...
        try:
            ...
            tmpBin = os.path.join(tmpDir, asset["name"])
            urllib.request.urlretrieve(asset["browser_download_url"], tmpBin)

            currentExe = sys.executable if not sys.executable.endswith("python") else None

            if isWindows:
                installDir = os.path.join(os.environ.get("LOCALAPPDATA", ""), "LockBox")
                dest = os.path.join(installDir, "lockbox.exe")
                os.makedirs(installDir, exist_ok=True)
                import shutil
                shutil.copy2(tmpBin, dest)
            else:
                dest = os.path.expanduser("~/.local/bin/lockbox")
                import shutil
                shutil.copy2(tmpBin, dest)
                os.chmod(dest, 0o755)

            console.print(Panel(
                f"[green]Updated to {latestTag}[/green]\n\nRestart your terminal for the new version to take effect.",
                border_style="green", box=box.ROUNDED,
            ))
        except Exception as e:
            console.print(Panel(f"[red]Update failed: {e}[/red]", border_style="red", box=box.ROUNDED))
            raise typer.Exit(1)
    else:
        console.print(Panel(
            f"[yellow]No binary found for your platform in {latestTag}.[/yellow]\n\n"
            "Update via pip instead:\n[bold cyan]pip install --upgrade lockbox[/bold cyan]",
            border_style="yellow", box=box.ROUNDED,
        ))
